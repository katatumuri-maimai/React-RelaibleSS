...
export function Kaigyo(msg){
    const InfoMsg = msg.split(/(<br>)/).map((item,index) => {
      return (
        <React.Fragment key={index}>
         {item.match(/<br>/) ? React.createElement('br') : item }
        </React.Fragment>
       );
    });
    return (InfoMsg);
...
export function CreatedSeq(){
 ...
 const N=20;
 const URL = Array.from(crypto.randomFillSync(new Uint8Array(N))).map((n)=>S[n%S.length]).join('');
